...
const OtpVerify = ({ phone, requestId, setIsSignedIn }) => {
            ...
            <div className='flex flex-col items-start w-[90%] gap-1'>
                ...
                <div className='w-full'>
                    <form onSubmit={handleSubmit} className='flex flex-col items-center justify-between mt-6'>
                        ...
                        <button type="submit" className={`bg-[#552583] w-full text-white rounded-xl px-6 py-3 font-bold text-[18px] ${loading ? 'disabled opacity-75': ''}`}>{loading ? 'Verifying' : 'Verify'}</button>
                    </form>
                </div>
            </div>
            <div className='mt-8 text-[#101920] underline font-light text-[16px] flex flex-col items-center justify-center gap-4 w-[90%]'>
                <button onClick={()=>setIsSignedIn(false)}>Resend OTP</button>
                <button onClick={()=>setIsSignedIn(false)}>Use another number</button>
            </div>
        </div>
    )
}
